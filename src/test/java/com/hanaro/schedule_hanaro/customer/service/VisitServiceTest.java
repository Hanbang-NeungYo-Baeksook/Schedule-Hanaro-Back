...
public class VisitServiceTest {
	...
	public void deleteVisitReservationTest() throws InterruptedException {
		...
		CustomUserDetails mockDetails = org.mockito.Mockito.mock(CustomUserDetails.class);
		org.mockito.Mockito.when(mockDetails.getUserId()).thenReturn(customer.getId());

		Authentication authentication = new UsernamePasswordAuthenticationToken(mockDetails, null, null);
		...
		Long visitId = visitService.addVisitReservation(VisitCreateRequest.builder()
			...
			.category(Category.SIGNIN.getCategory())
	...
	public void addVisitRequest100Test() throws Exception {
		...
		Branch branch = branchRepository.findByName("NormalTestBranch").orElseThrow();
		int initNum = csVisitRepository.findByBranchIdAndDate(branch.getId(), LocalDate.now())
			.orElseThrow()
			.getTotalNum();
		...
		CsVisit csVisit = csVisitRepository.findByBranchIdAndDate(branch.getId(), LocalDate.now()).orElseThrow();
		assertThat(csVisit.getTotalNum() - initNum).isEqualTo(threadCount);
	}
